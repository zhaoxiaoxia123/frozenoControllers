...
export class AssetsIssueComponent implements OnInit {
  ...
  getAssetsListDefault(){
    this.globalService.httpRequest('get','getAssetsDefault?type=department&assets_status='+this.assets_status_default+'&sid='+this.cookieStore.getCookie('sid'))
        .subscribe((data)=>{
  ...
  geteUserList(obj,type:number) {
    ...
    if(department_id != 0) {
      this.globalService.httpRequest('get','getAssetsDefault?type=user&department_id=' + department_id + '&sid=' + this.cookieStore.getCookie('sid'))
          .subscribe((data) => {
            this.userList = data;
